...
SELECT CAST(MAX(TMG.IDMatrikkelGate) as nvarchar(200)) allocationTemplateId, MAX(T.GUIDTjeneste) serviceReference, max(T.Navn) serviceName,
	(SELECT CAST(OtherTMG.IDPunkt as nvarchar(200)) "id", (OtherTMG.IDPunktRolle + 1) "priority" FROM TilordningMatrikkelGate OtherTMG WHERE
	...
	FROM TilordningMatrikkelGate TMG
	INNER JOIN MatrikkelGate MG ON TMG.IDMatrikkelGate = MG.IDMatrikkelGate
	INNER JOIN BossIDTjeneste T ON TMG.IDTjeneste = T.IDTjeneste 	   
