...
WITH ContRows (id, name, tag, services, parentId, type) AS (
	SELECT TjenesteIDEnhet as id, Navn as name, merkelapp as tag, 
		(dbo.ufnToRawJsonArray((SELECT 'S' + convert(nvarchar, TE2.IDTjeneste) as serviceId FROM [BossID].[dbo].TommeEnhet TE2 WHERE TE2.TjenesteIDEnhet = TE.TjenesteIDEnhet FOR JSON PATH)
			, 'serviceId')) services,
		'CONTAINER_ROOT' as parentId,
		'CONTAINER' as type
		FROM [BossID].[dbo].TommeEnhet TE WHERE TE.IDTommeEnhet IN (SELECT MAX(IDTommeEnhet) FROM [BossID].[dbo].TommeEnhet TEG GROUP BY TjenesteIDEnhet HAVING MAX(IDPunkt) IS NULL)
	UNION
