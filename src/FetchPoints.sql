...
WITH AccessPoint (Id, bossIdId, Guid, IDPunktType, IDFraksjon, IDPunktEgenskap, IDPunktKundeTyper, ServiceGuid, ServiceId, ServiceRef, ParentId, tag, name, description, geoLocation, status, oldCustomerFn) 
	  ...
	  status as 'status'
	  FROM AccessPoint AP
	  INNER JOIN FraksjonsType FT ON ft.IDFraksjon = AP.IDFraksjon
	  INNER JOIN PunktEgenskap PE ON PE.IDPunktEgenskap = AP.IDPunktEgenskap
	  LEFT OUTER JOIN PunktType PT ON pt.IDPunktType = AP.IDPunktType -- Note: "terminal" = hardcoded to 10 above
	  LEFT OUTER JOIN BossIDTjeneste BS ON BS.IDTjeneste = AP.bossIdId AND AP.IDPunktType = 10
	  LEFT OUTER JOIN BrikkeTyper BT ON BT.IDBrikkeType = BS.IDBrikkeType AND AP.IDPunktType = 10
	  LEFT OUTER JOIN PunktKundeType PKT ON PKT.IDPunktKundeTyper=AP.IDPunktKundeTyper
